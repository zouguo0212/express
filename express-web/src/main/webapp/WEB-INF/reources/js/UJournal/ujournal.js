...
var ujournalGrid = new Ext.grid.GridPanel({
			id : 'ujournalGrid',
			plugins : [rowEditing],
			store : store,
			region : 'center',
...
ujournalGrid.getSelectionModel().on('selectionchange',
		function(selModel, selections) {
	          ujournalGrid.down('#delete').setDisabled(selections.length === 0);
		});
...
var clearForm = function() {
	...
	ujournalForm.getForm().reset();
}

var queryForm = function() {
	Ext.Msg.alert('查询', '将开始执行查询！');
}
var ujournalForm = new Ext.form.FormPanel({
			title : '信息查询',
			width : 200,
			height : 200,
			frame : true,
...
Ext.application({
			...
			launch : function() {
				Ext.create('Ext.container.Viewport', {
							layout : 'border',
							items : [ujournalForm, ujournalGrid]
						});
			}
		});
