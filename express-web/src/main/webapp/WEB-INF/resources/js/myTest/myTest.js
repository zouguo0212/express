...
Ext.define('myFinance.MyFinanceModel', {
...
var store = new Ext.data.Store({
    autoLoad : true,
    autoSync : true,// 需要同步
    model : 'myFinance.MyFinanceModel',
    proxy : {
        type : 'rest',
        url : './.json',
        reader : {
...
var myFinanceGrid = new Ext.grid.GridPanel({
    id : 'myFinanceGrid',
    plugins : [rowEditing],
    store : store,
    region : 'center',
...
myFinanceGrid.getSelectionModel().on('selectionchange',
    function(selModel, selections) {
        myFinanceGrid.down('#delete').setDisabled(selections.length === 0);
    });
...
var myFinanceForm = new Ext.form.FormPanel({
    title : '信息查询',
    width : 200,
    height : 200,
    frame : true,
...
Ext.application({
    ...
    launch : function() {
        Ext.create('Ext.container.Viewport', {
            layout : 'border',
            items : [myFinanceForm, myFinanceGrid]
        });
    }
});
